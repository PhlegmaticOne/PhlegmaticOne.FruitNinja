...
namespace Concrete.Commands.BlockCommands
{
    public class EnableSamuraiModeCommand : IBlockOnDestroyCommand
    {
        ...
        public void OnDestroy(Block entity, BlockDestroyContext destroyContext)
        {
            ...
            _healthController.DisableHeartRemoving(_duration + 1f);
            _spawningSystem.ChangeSpawnDifficulty(samuraiSpawningDifficulty, _duration - MinusLastSpawnTime());
        }

        private float MinusLastSpawnTime()
        {
            return _spawnSystemConfiguration.SpawnPackageIntervals.Max / _decreasePackageIntervalsBy;
        }
    }
}